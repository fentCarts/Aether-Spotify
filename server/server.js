...
app.get("/api/search", requireAuth, (req, res) => {
  const { q, type = "track,artist,album,playlist", limit = 12 } = req.query;
  if (!q) return res.json({});
  spotify(req, res, "GET", "/search", { query: { q, type, limit } });
});
...
app.get("/api/me/tracks/contains", requireAuth, (req, res) => {
  spotify(req, res, "GET", "/me/tracks/contains", { query: { ids: req.query.ids } });
});
...
app.put("/api/me/tracks", requireAuth, (req, res) => {
  spotify(req, res, "PUT", "/me/tracks", { body: req.body }); // like
});
app.delete("/api/me/tracks", requireAuth, (req, res) => {
  spotify(req, res, "DELETE", "/me/tracks", { body: req.body }); // unlike
});
