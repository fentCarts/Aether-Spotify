...
//   3. Proxying LRCLIB synced-lyrics lookups
//
// Tokens (Spotify access/refresh) live only in this process — server-side
// session storage — never in the browser, never in a cookie the browser can
// read, and never in client-side JS. The one deliberate exception is
// /api/player-token (see comment above that route) which hands the frontend
// a short-lived Spotify access token so the Web Playback SDK can stream
// audio — that token cannot do anything the SDK doesn't already need it to
// do, and it expires in ~1 hour. LRCLIB needs no key at all.
...
app.post("/api/me/playlists", requireAuth, (req, res) => {
  // body: { name, description, public }
  spotify(req, res, "POST", "/me/playlists", { body: req.body });
});
app.post("/api/playlists/:id/items", requireAuth, (req, res) => {
  // body: { uris: [...] }
  spotify(req, res, "POST", `/playlists/${req.params.id}/items`, { body: req.body });
});
...
// time_range: short_term (~4wk) | medium_term (~6mo) | long_term (years). Defaults to short_term.
app.get("/api/me/top/artists", requireAuth, (req, res) => {
  const time_range = ["short_term", "medium_term", "long_term"].includes(req.query.time_range) ? req.query.time_range : "short_term";
  spotify(req, res, "GET", "/me/top/artists", { query: { limit: req.query.limit || 10, time_range } });
});

app.get("/api/me/top/tracks", requireAuth, (req, res) => {
  const time_range = ["short_term", "medium_term", "long_term"].includes(req.query.time_range) ? req.query.time_range : "short_term";
  spotify(req, res, "GET", "/me/top/tracks", { query: { limit: req.query.limit || 10, time_range } });
});

// Artists the user follows — still available post-Feb-2026 (unlike track/album "Get Several" endpoints).
app.get("/api/me/following", requireAuth, (req, res) => {
  spotify(req, res, "GET", "/me/following", { query: { type: "artist", limit: req.query.limit || 30 } });
});

// An artist's albums — used to power "new releases from artists you follow" now that
// GET /browse/new-releases has been removed from the Web API.
app.get("/api/artists/:id/albums", requireAuth, (req, res) => {
  spotify(req, res, "GET", `/artists/${req.params.id}/albums`, { query: { limit: req.query.limit || 6, include_groups: "album,single" } });
});
...
/* ------------------------- LRCLIB lyrics proxy ------------------------- */
// LRCLIB (lrclib.net) is a free, community-run, open lyrics database — no
// API key or account required. Coverage isn't as complete as a licensed
// provider, and there's no uptime/rate-limit guarantee since it's a
// volunteer-run free service, but it's a solid zero-cost option and often
// does have synced (LRC-format) lyrics, not just plain text.
//
// GET /get requires an exact-ish match (track + artist [+ album/duration
// for accuracy]) and 404s if nothing matches closely enough. We fall back
// to GET /search, which is fuzzier and returns a ranked list of candidates,
// and take the first result that actually has lyrics.
app.get("/api/lyrics", requireAuth, async (req, res) => {
  const { artist, title, duration_sec } = req.query;
  ...
  try {
    ...
    if (duration_sec) getUrl.searchParams.set("duration", duration_sec);

    let track = null;
    const getRes = await fetch(getUrl, { headers: { "User-Agent": "Aether (https://github.com/) - hobby project" } });
    if (getRes.ok) {
      track = await getRes.json();
    } else {
      // Exact match failed — try the fuzzier search endpoint instead.
      const searchUrl = new URL("https://lrclib.net/api/search");
      searchUrl.searchParams.set("track_name", title);
      searchUrl.searchParams.set("artist_name", artist);
      const searchRes = await fetch(searchUrl, { headers: { "User-Agent": "Aether (https://github.com/) - hobby project" } });
      if (searchRes.ok) {
        const results = await searchRes.json();
        track = (results || []).find((t) => t.syncedLyrics || t.plainLyrics) || null;
      }
    }

    if (!track || (!track.syncedLyrics && !track.plainLyrics)) {
      return res.status(404).json({ error: "lyrics_unavailable", hint: "No lyrics found for this track on LRCLIB." });
    }

    if (track.syncedLyrics) {
      return res.json({ format: "lrc", synced: true, lrc: track.syncedLyrics, lines: parseLRC(track.syncedLyrics) });
    }
