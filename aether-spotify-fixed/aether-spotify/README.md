...
- **Synced lyrics** via Musixmatch, proxied server-side so the API key never reaches the browser

Architecture, in one line: browser (frontend) → your Node server (holds all tokens/keys) → Spotify / Musixmatch. The frontend never talks to Spotify or Musixmatch directly.
...
## 2. Get a Musixmatch key (synced lyrics)

1. Sign up at https://developer.musixmatch.com and grab a free API key to start.
2. The free/community tier can look up tracks and return partial plain lyrics, but **synced ("subtitle") lyrics require a commercial/partner license** from Musixmatch — without one, `matcher.subtitle.get` will typically 401. Reach out to their partnerships team before you rely on this for real users. The code is written against their subtitle endpoint and parses standard `.lrc` timestamps, so swapping in a different licensed provider later just means changing the one `/api/lyrics` handler in `server/server.js`.
...
cd server
cp .env.example .env
# edit .env: SPOTIFY_CLIENT_ID, SPOTIFY_REDIRECT_URI, MUSIXMATCH_API_KEY, SESSION_SECRET, PUBLIC_URL
```
