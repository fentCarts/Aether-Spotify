...
// track.subtitle.get (synced, line-by-line lyrics) requires a Musixmatch
// commercial license. Without one this returns 401/403 for basically every
// track — so on a free/community API key, we fall back to matcher.lyrics.get
// (plain, unsynced lyrics), which the free tier does expose, capped to a
// partial excerpt by Musixmatch itself for non-commercial keys. We pass
// their restricted flag and copyright line through unchanged rather than
// stripping them, per Musixmatch's API terms.
app.get("/api/lyrics", requireAuth, async (req, res) => {
  ...
  try {
    ...
    if (header?.status_code === 200) {
      ...
      return res.json({ format: "lrc", synced: true, lrc, lines: parseLRC(lrc) });
    }

    if (header?.status_code === 401 || header?.status_code === 403) {
      const plain = await fetchPlainLyrics(artist, title);
      if (plain) return res.json(plain);
    }
    ...
    return res.status(header?.status_code === 401 ? 402 : 404).json({
      ...
      hint:
        header?.status_code === 401
          ? "Musixmatch returned 401 for synced subtitles, and no plain lyrics were found for this track either."
...
async function fetchPlainLyrics(artist, title) {
  const plainUrl = new URL("https://api.musixmatch.com/ws/1.1/matcher.lyrics.get");
  plainUrl.searchParams.set("q_track", title);
  plainUrl.searchParams.set("q_artist", artist);
  plainUrl.searchParams.set("apikey", MUSIXMATCH_API_KEY);

  const r = await fetch(plainUrl);
  const j = await r.json();
  const header = j?.message?.header;
  const lyrics = j?.message?.body?.lyrics;
  if (header?.status_code !== 200 || !lyrics?.lyrics_body) return null;

  return {
    format: "plain",
    synced: false,
    restricted: !!lyrics.restricted, // 1 when Musixmatch has capped this to a snippet for a non-commercial key
    copyright: lyrics.lyrics_copyright || null,
    lines: lyrics.lyrics_body.split("\n").filter((l) => l.trim()).map((text) => ({ text })),
  };
}
